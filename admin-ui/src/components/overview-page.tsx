...
function presetStartDate(range: StatsRange, endDate: string): string {
  const days = range === '24h' ? 1 : range === '7d' ? 6 : range === '30d' ? 29 : 365
...
export function OverviewPage() {
  ...
  const { data: series } = useTimeSeries(filters.timeFilter)
  const { data: byModel } = useByModel(filters.timeFilter)
  const { data: byCred } = useByCredential(filters.timeFilter)
  ...
  return (
    <div className="animate-fade-in">
      ...
      <StatsCards
        ...
        totalCredentials={overview?.totalCredentials ?? overview?.activeCredentials ?? 0}
        stats={rangeStats}
...
function useOverviewFilters() {
  ...
  const selectPresetRange = (range: StatsRange) => {
    ...
    const nextGranularity: StatsGranularity =
      range === 'all' ? 'day' : range === '24h' ? 'hour' : draftGranularity
    setCustomStartDate(startDate)
    setCustomEndDate(endDate)
    setDraftRange(range)
    setDraftGranularity(nextGranularity)
    setTimeFilter(customTimeFilter(startDate, endDate, nextGranularity))
  }
  const updateGranularity = (value: StatsGranularity) => {
    setDraftGranularity(value)
    setTimeFilter(customTimeFilter(customStartDate, customEndDate, value))
  }
  return {
    applyCustomRange,
    customEndDate,
    ...
    setDraftGranularity: updateGranularity,
    timeFilter,
  }
}
...
function StatsCards({
  ...
  const cards = [
    ...
    },
    {
      ...
      label: '可用凭据',
      meta: '当前可用上游',
      value: formatNumber(activeCredentials),
      className: 'col-span-2 max-[360px]:col-span-1 lg:col-span-1',
      extra: (
        <span className="text-[11px] text-muted-foreground">
          共 {formatNumber(totalCredentials)}
        </span>
      ),
    },
  ]
...
function TrendCard({
  ...
  const chartKey = timeLabel(timeFilter)
  return (
    <Card className="mb-6">
...
function PresetRangeButtons({
  ...
  return (
    <div className="grid grid-cols-4 gap-1 rounded-md border border-border/60 p-0.5 lg:flex lg:items-center">
      {RANGES.map((r) => (
        <Button
          key={r.value}
          size="sm"
...
function GranularitySelect({
  ...
  return (
    <select
      value={value}
      onChange={(e) => onChange(e.target.value as StatsGranularity)}
      className="h-8 w-full rounded-md border border-border/60 bg-background px-2 text-xs lg:w-[96px]"
    >
      {GRANULARITIES.map((g) => (
...
function DistributionPanels({
  ...
  return (
    <div className="mb-6 grid gap-4 lg:grid-cols-2">
      ...
      <CredentialPanel data={byCred} hasCalls={hasCalls} />
    </div>
  )
}
  ...
  return (
    <Card>
      <CardContent className="p-4 sm:p-5">
        ...
        {hasCalls && data.length === 0 && (
          <p className="mb-3 rounded-md bg-amber-500/10 px-2.5 py-1.5 text-[11px] text-amber-700 dark:text-amber-400">
            历史请求没有写入凭据 ID。之后的新请求会按凭据聚合显示。
          </p>
        )}
